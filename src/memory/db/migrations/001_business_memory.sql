...
CREATE INDEX IF NOT EXISTS idx_conv_lead ON conversations(lead_memory_id);
CREATE INDEX IF NOT EXISTS idx_conv_zoho ON conversations(zoho_lead_id);
...
CREATE INDEX IF NOT EXISTS idx_fu_lead ON follow_ups(lead_memory_id);
CREATE INDEX IF NOT EXISTS idx_fu_zoho ON follow_ups(zoho_lead_id);
CREATE INDEX IF NOT EXISTS idx_fu_status ON follow_ups(status);
CREATE INDEX IF NOT EXISTS idx_fu_sched ON follow_ups(scheduled_at ASC NULLS LAST);
...
DO $$ DECLARE t TEXT;
BEGIN
  FOREACH t IN ARRAY ARRAY['lead_memory','conversations','retell_calls','email_events','salesiq_chats','crm_tasks','crm_notes','follow_ups']
  LOOP
