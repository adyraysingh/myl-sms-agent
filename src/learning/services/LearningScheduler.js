...
async function evaluateCopilotQuality() {
  ...
  try {
    // Count responses and feedback from last 24h
...
async function updatePromptStats() {
  // For each active prompt, update call stats from related tables
  const promptUpdates = [
...
async function detectDrift() {
  try {
    // Compare last 7 days vs prior 7 days for each module
    const r = await pool.query(
      `SELECT
         source_module,
...
async function generateMonthlyReport() {
  try {
    ...
    // Save as optimization suggestion with monthly report content
    await pool.query(
