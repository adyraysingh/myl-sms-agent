    ...
    try {
      ...
      if (lastCycle.rows.length > 0) {
        ...
        if (lc.status === 'failed' || hoursSinceCycle > 48) {
          await OperationalIntelligenceAgent._saveRec(runId, { rec_type: 'learning_cycle_issue', title: lc.status === 'failed' ? 'Learning cycle failed: ' + (lc.error_message||'unknown error').slice(0,80) : 'Learning cycle not run in ' + Math.round(hoursSinceCycle) + ' hours', description: 'Last learning cycle status: ' + lc.status + '. Hours since last completion: ' + Math.round(hoursSinceCycle), evidence: JSON.stringify([{ signal: 'status', value: lc.status }, { signal: 'hours_since_completion', value: Math.round(hoursSinceCycle) }]), reasoning: 'The learning engine must run regularly to maintain AI accuracy calibration.', confidence: 85, affected_lead_id: null, affected_resource: JSON.stringify({ cycle_type: lc.cycle_type, status: lc.status, hours_ago: Math.round(hoursSinceCycle) }), recommended_action: 'Trigger manual learning cycle via /api/learning/run. Investigate scheduler if consistently failing.', expected_impact: 'Learning cycles are critical for AI confidence calibration and continuous improvement', priority: 'medium' });
