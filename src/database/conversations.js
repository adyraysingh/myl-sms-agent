...
async function createConversation(leadData) {
    const leadId = typeof leadData === 'object' ? leadData.leadId || leadData : leadData;
...
async function updateConversationStatus(conversationId, status, summary) {
    const result = await pool.query(
          `UPDATE conversations SET status = $1, summary = COALESCE($3, summary), updated_at = NOW()
               WHERE id = $2 RETURNING *`,
          [status, conversationId, summary || null]
        );
    return result.rows[0];
}
