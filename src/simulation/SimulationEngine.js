'use strict';
/**
 * Phase 3.5 — SimulationEngine.js
 * Longitudinal Learning Validation
 *
 * KEY FIX vs Phase 3.4:
 * - Injects synthetic conversation_analysis evidence BEFORE qualification runs
 * - ContextEngine now scores >= 50 for every lead (passes the 20-pt threshold)
 * - PredictionPublisher fires automatically via production QualificationProcessor
 * - No backfill needed — predictions are created by the real pipeline
 * - Supports cohort comparison: first 200 vs last 200 predictions
 * - Runs 3 learning cycles to show measurable improvement
 */
...
async function ingestLeadViaProduction(index, profile, batchTag) {
  ...
  const zohoPayload = {
    ...
    email: 'sim35_' + index + '_' + rand(100, 999) + '@' + country.toLowerCase().replace(/\s/g, '') + '-sim35.com',
    phone: '+1555' + String(rand(1000000, 9999999)),
    Lead_Source: 'Simulation_35',
    company: brandType + ' Brand ' + index,
    Company: brandType + ' Brand ' + index,
    Description: 'Phase 3.5 simulation. Profile: ' + profile.type + '. Country: ' + country + '. Budget: $' + budget + '. Timeline: ' + timeline,
    budget, timeline, country, brand_type: brandType,
    Owner: { id: 'owner_sim35', name: 'Simulation Manager' }
  };
  ...
  EventOrchestrator.emit('lead.created', {
    ...
    email: zohoPayload.email, phone: zohoPayload.phone
  });
  return { memory, profile, country, brandType, budget, timeline, zohoLeadId };
}

async function injectConversationEvidence(memory, profile, budget, timeline) {
  const conversationId = 'sim35_conv_' + memory.id + '_' + Date.now();
  ...
  await pool.query(
    ...
    [
      ...
      'Lead interested in ' + (memory.brand_name || brandType || 'fashion') + ' manufacturing services',
      profile.qualifyRate >= 0.7 ? 'positive_progression' : profile.qualifyRate >= 0.4 ? 'neutral_progression' : 'stalled',
      'Phase 3.5 synthetic conversation analysis for lead ' + memory.id,
...
async function simulateOnboarding(memory, profile) {
  ...
  try {
    ...
    return true;
  } catch (e) { console.error('[Sim35] Onboarding error:', e.message); return false; }
}
...
async function simulateRevenueEvent(memory, dealValue) {
  try {
    ...
    const result = await RevenueForecaster.evaluateForecast(forecast.forecast_id, {
      ...
      notes: 'Sim35: revenue event ' + memory.id
    });
    setImmediate(() =>
      PredictionPublisher.linkOutcome({
        ...
        notes: 'Sim35: revenue received', source: 'simulation'
      }).catch(() => {})
    );
    return { actual_revenue: actualRevenue, result };
  } catch (e) { console.error('[Sim35] Revenue error:', e.message); return null; }
}
...
async function waitForQueueDrain(queueName, maxWaitMs) {
  ...
  while (Date.now() - startWait < maxWaitMs) {
    try {
      ...
      console.log('[Sim35] Queue', queueName, 'pending:', pending);
...
async function runSimulation(config) {
  config = config || {};
  const totalLeads = config.total_leads || 600;
  ...
  const batchTag = config.batch_tag || 'SIM35';
  ...
  console.log('[Sim35] Phase 3.5 starting:', totalLeads, 'leads via production pipeline + evidence injection');
  ...
  } catch (e) { console.error('[Sim35] Forecast init error:', e.message); }
  ...
  for (let i = 0; i < totalLeads; i += batchSize) {
    ...
    await Promise.all(batch.map(async function(item) {
      ...
      try {
        const { memory, budget, timeline } = await ingestLeadViaProduction(index, profile, batchTag);
        stats.leads_ingested++;
        await injectConversationEvidence(memory, profile, budget, timeline);
        ...
        await WorkerRegistry.enqueueDecision({
          ...
          trigger_source: 'sim35', trigger_data: { profile: profile.type }
        });
      ...
      } catch (e) { stats.errors++; console.error('[Sim35] Lead', index, 'error:', e.message); }
    }));

    if (delayMs > 0) await sleep(delayMs);
    if (i % (batchSize * 5) === 0) {
      console.log('[Sim35] Progress:', Math.min(i + batchSize, totalLeads), '/', totalLeads, '| Ingested:', stats.leads_ingested, '| Errors:', stats.errors);
    }
  }

  console.log('[Sim35] All leads ingested. Waiting for qualification queue to drain...');
  await waitForQueueDrain('qualification', 120000);
  await sleep(5000);

  console.log('[Sim35] Running', copilotCount, 'CEO Copilot questions...');
  const sessionId = 'sim35_session_' + Date.now();
  for (let q = 0; q < copilotCount; q++) {
    try {
      await ExecutiveCopilot.ask({ question: pick(COPILOT_QUESTIONS), session_id: sessionId, context: { source: 'simulation_35' } });
      stats.copilot_questions_asked++;
      await sleep(100);
    } catch (_) { stats.errors++; }
  }

  console.log('[Sim35] Triggering learning cycle 1 of 3 (daily)...');
  try { await LearningScheduler.runManual('daily'); } catch (e) { console.error('[Sim35] Learning 1 error:', e.message); }
  await sleep(2000);
  console.log('[Sim35] Triggering learning cycle 2 of 3 (daily)...');
  try { await LearningScheduler.runManual('daily'); } catch (e) { console.error('[Sim35] Learning 2 error:', e.message); }
  await sleep(2000);
  console.log('[Sim35] Triggering learning cycle 3 of 3 (weekly)...');
  try { await LearningScheduler.runManual('weekly'); } catch (e) { console.error('[Sim35] Learning 3 error:', e.message); }
  ...
  console.log('[Sim35] COMPLETE. Duration:', stats.duration_seconds, 's. Leads:', stats.leads_ingested, '| Predictions:', stats.total_predictions);
  return stats;
}

async function validateSimulation() {
  const results = { checks: {}, phase: '3.5' };
  try {
    ...
    const r8 = await pool.query("SELECT COUNT(*) as cnt FROM lead_memory WHERE zoho_lead_id LIKE 'SIM35_%'");
    results.checks.sim35_lead_memory = parseInt(r8.rows[0].cnt);
    results.dashboard_populated = results.checks.sim35_lead_memory > 0;
    const r9 = await pool.query("SELECT COUNT(*) as cnt FROM conversation_analysis WHERE lead_id IN (SELECT id FROM lead_memory WHERE zoho_lead_id LIKE 'SIM35_%')");
    results.checks.conversation_analyses = parseInt(r9.rows[0].cnt);
    const r10 = await pool.query("SELECT COUNT(*) as cnt FROM lead_qualification WHERE lead_id IN (SELECT id FROM lead_memory WHERE zoho_lead_id LIKE 'SIM35_%')");
    results.checks.sim35_qualifications = parseInt(r10.rows[0].cnt);
    results.qualifications_exist = results.checks.sim35_qualifications > 0;
    try {
