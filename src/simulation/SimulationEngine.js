...
*
* KEY CHANGES vs Phase 3.6:
* 1. POOL CONSOLIDATION (Task 2): PlatformModel, PlatformMonitor, revenue.routes,
*    platform.routes all migrated to shared pool. Application now uses ONE Pool instance.
*    Before: 8 separate Pool() instances competing for Railway's 15-slot limit.
*    After: 1 shared Pool(max:20) — all modules share it cleanly.
* 2. BATCH TAG updated to SIM37 for fresh load test validation.
* 3. All Phase 3.6 fixes remain: race condition, schema, scope, forecast fixes.
*/
...
const conversationId = 'sim37_conv_' + memory.id + '_' + Date.now();
...
'Phase 3.7 synthetic conversation analysis for lead ' + memory.id,
...
const batchTag = config.batch_tag || 'SIM37';
...
console.log('[Sim37] batch_size=' + batchSize + ' delay_ms=' + delayMs + ' pool.max=20 (single shared pool)');
...
const sessionId = 'sim37_session_' + Date.now();
...
const results = { checks: {}, phase: '3.7' };
try {
...
const r8 = await pool.query("SELECT COUNT(*) as cnt FROM lead_memory WHERE zoho_lead_id LIKE 'SIM37_%'");
results.checks.sim37_lead_memory = parseInt(r8.rows[0].cnt);
results.dashboard_populated = results.checks.sim37_lead_memory > 0;
const r9 = await pool.query("SELECT COUNT(*) as cnt FROM conversation_analysis WHERE lead_id IN (SELECT id FROM lead_memory WHERE zoho_lead_id LIKE 'SIM37_%')");
results.checks.conversation_analyses = parseInt(r9.rows[0].cnt);
const r10 = await pool.query("SELECT COUNT(*) as cnt FROM lead_qualification WHERE lead_id IN (SELECT id FROM lead_memory WHERE zoho_lead_id LIKE 'SIM37_%')");
...
// Pool health check
try {
const r13 = await pool.query("SELECT COUNT(*) as active, (SELECT COUNT(*) FROM pg_stat_activity WHERE datname=current_database() AND state='idle') as idle FROM pg_stat_activity WHERE datname=current_database() AND state='active'");
...
module.exports = { runSimulation, validateSimulation, analyzeLearningImprovement, compareEarlyvLate };'use strict';
/**
