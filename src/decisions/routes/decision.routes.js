...
// Phase 3.2: Auto-link decision prediction outcome (fire-and-forget)
if (updated) {
setImmediate(() => PredictionPublisher.autoLinkOutcome({
module: 'decision_engine',
lead_id: updated.lead_id || null,
outcome_type: 'decision_completed',
outcome_value: { decision_id: updated.decision_id, decision_type: updated.decision_type, execution_result: execution_result || 'Completed', completed_at: new Date().toISOString() },
is_correct: true,
accuracy_score: 1.0,
notes: 'Decision executed successfully'
}).catch(e => console.error('[OutcomeLinker] decision_completed failed:', e.message)));
}
res.json({ success: true, decision: updated });
} catch (err) {
res.status(500).json({ success: false, error: err.message });
...
// Phase 3.2: Auto-link decision prediction outcome as dismissed (fire-and-forget)
if (updated) {
setImmediate(() => PredictionPublisher.autoLinkOutcome({
module: 'decision_engine',
lead_id: updated.lead_id || null,
outcome_type: 'decision_dismissed',
outcome_value: { decision_id: updated.decision_id, decision_type: updated.decision_type, dismissed_reason: dismissed_reason || 'Not applicable', dismissed_at: new Date().toISOString() },
is_correct: false,
accuracy_score: 0,
notes: 'Decision dismissed: ' + (dismissed_reason || 'Not applicable')
}).catch(e => console.error('[OutcomeLinker] decision_dismissed failed:', e.message)));
}
res.json({ success: true, decision: updated });
} catch (err) {
res.status(500).json({ success: false, error: err.message });
