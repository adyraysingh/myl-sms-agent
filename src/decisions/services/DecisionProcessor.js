...
class DecisionProcessor {
  ...
  static async _processItem(item) {
        try {
          ...
          let contextResult;
                try {
                          ...
                          if (ctxErr.code === 'INSUFFICIENT_CONTEXT') {
                                      console.warn('[DP] INSUFFICIENT CONTEXT for lead ' + resolvedLeadId);
                                      await AIDecision.updateQueueStatus(item.queue_id, 'insufficient_context', 'Insufficient evidence. Score: ' + ctxErr.evidenceScore);
                                      return;
                          }
                          throw ctxErr;
                }

          const leadData = DecisionProcessor._buildLeadDataFromContext(resolvedLeadId, resolvedZohoId, item.trigger_event, item.trigger_source, contextResult);
          ...
          for (const d of result.decisions) {
                    ...
                    saved.push(dec);
                    // Phase 3.1: Auto-publish decision prediction (fire-and-forget)
                  setImmediate(() => PredictionPublisher.decision(resolvedLeadId, dec).catch(() => {}));
  ...
  static _buildLeadDataFromContext(lead_id, zoho_lead_id, trigger_event, trigger_source, ctx) {
        ...
        const memory = {
                id: lead_id, zoho_lead_id, customer_name: mem.name, name: mem.name,
                email: mem.email, phone: mem.phone, country: mem.country,
                brand_name: mem.brand_name, lead_source: mem.lead_source,
                crm_owner: mem.crm_owner, last_activity_at: mem.last_activity
        };
        const qualification = qual ? {
                qualification_category: qual.category, onboarding_score: qual.onboarding_score,
                onboarding_probability: qual.onboarding_probability, trust_score: qual.trust_score,
                budget_confidence: qual.budget_confidence, timeline_confidence: qual.timeline_confidence,
                brand_readiness: qual.brand_readiness, confidence_score: qual.confidence_score,
                qualification_reason: qual.overall_reasoning, missing_information: (qual.qualification_gaps || []).map(g => g.gap)
        } : null;
        const conversations = context.all_conversations.map(c => ({
                summary: c.intent, customer_intent: c.intent, sentiment: c.sentiment,
                trust_score: c.trust, buying_intent_score: c.buying_intent,
                positive_buying_signals: [], objections: [], recommended_next_step: null
        }));
        if (conversations.length > 0) {
                conversations[0] = {
                          summary: conv.summary, customer_intent: conv.customer_intent,
                          sentiment: conv.sentiment, trust_score: conv.trust_score,
                          buying_intent_score: conv.buying_intent_score,
                          positive_buying_signals: conv.positive_buying_signals || [],
                          objections: conv.objections || [], recommended_next_step: conv.recommended_next_step
                };
        }
        return {
                lead_id, crm_owner: mem.crm_owner || null, trigger_event, trigger_source, memory, conversations, qualification,
                ...
                evidence_score: ctx.evidence_score, missing_signals: ctx.missing_signals
        };
  }

  static async _storeInTimeline(lead_id, zoho_lead_id, decisions, result) {
        try {
                if (!decisions.length) return;
                const s = decisions.map(d => d.priority + ':' + d.decision_type).join(';');
                // Fix: use lead_memory_id and zoho_lead_id columns (correct schema)
          // lead_id parameter is already the lead_memory.id UUID
          await pool.query(
                    'INSERT INTO lead_events (lead_memory_id, zoho_lead_id, event_type, source, summary, metadata) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT DO NOTHING',
                    [lead_id, zoho_lead_id || '', 'ai_decision_generated', 'decision_engine', 'AI decisions: ' + s, JSON.stringify({ count: decisions.length, urgency: result.urgency_level })]
                  );
