...
class BusinessInvestigationEngine {
  // Run a specific investigation by question
  static async investigate(question, trigger_event = null, investigation_type = null) {
    const startTime = Date.now();
    console.log('[BusinessInvestigationEngine] Starting investigation:', question);

    try {
      // Gather evidence from all modules
      const evidence = await BusinessInvestigationEngine._gatherEvidence(investigation_type);

      // Build prompt
      const systemPrompt = [
        'You are an elite business investigation AI for MakeYourLabel, a premium private label clothing manufacturer.',
        'MakeYourLabel helps brands launch with low MOQ, sampling, custom branding, tech packs, packaging, and full manufacturing.',
        '',
        'INVESTIGATION MANDATE:',
        'Investigate the business question using the evidence provided.',
        'Identify root causes. Do not just describe symptoms.',
        'Provide specific, actionable conclusions backed by the data.',
        'Every claim must reference specific evidence.',
        '',
        'OUTPUT FORMAT (JSON):',
        '{',
        '  "root_cause": "The specific root cause identified",',
        '  "conclusion": "Executive-level conclusion with evidence",',
        '  "severity": "critical|high|medium|low",',
        '  "business_impact": "Quantified or qualified business impact",',
        '  "recommendations": [',
        '    {"action": "Specific action", "owner": "Who", "timeline": "When", "expected_impact": "What changes"}',
        '  ],',
        '  "affected_owners": ["owner_id_1"],',
        '  "confidence_score": 85',
        '}'
      ].join('
');
...
');
      ...
      try {
        ...
        result = {
          ...
          severity: 'medium',
          business_impact: 'See conclusion',
          recommendations: [],
          affected_owners: [],
          confidence_score: 70
        };
  ...
  // Run automatic daily investigations
  static async runDailyInvestigations() {
    const questions = [
      { q: 'Why are onboardings lower today compared to yesterday?', type: 'onboarding_drop' },
  ...
  static async _gatherEvidence(investigation_type) {
    ...
    const [quals, decisions, convAnalysis, salesPerf, leads] = await Promise.allSettled([
      pool.query(`SELECT qualification_category, COUNT(*) as count, AVG(onboarding_score) as avg_score
                  FROM lead_qualification GROUP BY qualification_category ORDER BY count DESC LIMIT 10`),
      pool.query(`SELECT decision_type, priority, status, COUNT(*) as count
                  FROM ai_decisions GROUP BY decision_type, priority, status ORDER BY count DESC LIMIT 20`),
      pool.query(`SELECT sentiment, conversation_outcome, COUNT(*) as count,
                  AVG(trust_score) as avg_trust, array_agg(DISTINCT jsonb_array_elements_text(objections)) as objections
                  FROM conversation_analysis
                  WHERE analyzed_at > NOW() - INTERVAL '7 days'
                  GROUP BY sentiment, conversation_outcome LIMIT 20`),
      pool.query(`SELECT owner_id, owner_name, productivity_score, onboarding_rate,
                  follow_up_completion_rate, follow_ups_missed, performance_trend
                  FROM sales_performance
                  WHERE period_date = CURRENT_DATE ORDER BY productivity_score DESC LIMIT 20`),
      pool.query(`SELECT qualification_category, COUNT(*) as count
                  FROM lead_memory lm
                  LEFT JOIN lead_qualification lq ON lq.lead_id = lm.lead_id
                  GROUP BY qualification_category LIMIT 10`)
    ]);
  ...
  static _detectType(question) {
    ...
    if (q.includes('executive') || q.includes('sales rep') || q.includes('loses')) return 'executive_performance';
    if (q.includes('objection')) return 'objection_spike';
    if (q.includes('product')) return 'product_performance';
    if (q.includes('follow-up') || q.includes('followup') || q.includes('drop-off')) return 'followup_dropoff';
