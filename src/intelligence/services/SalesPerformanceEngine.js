  ...
  static async _getActiveOwners(date) {
    const result = await pool.query(
      `SELECT DISTINCT
       ...
       FROM lead_memory lm
       LEFT JOIN conversation_analysis ca ON ca.lead_id = lm.lead_id
  ...
  static async _getCallMetrics(owner_id, start, end) {
    const result = await pool.query(
      `SELECT
       ...
       JOIN lead_memory lm ON lm.lead_id = rc.lead_id
       WHERE lm.owner_id = $1
  ...
  static async _getChatMetrics(owner_id, start, end) {
    const result = await pool.query(
      `SELECT COUNT(*) as count
       ...
       JOIN lead_memory lm ON lm.lead_id = sc.lead_id
  ...
  static async _getDecisionMetrics(owner_id, start, end) {
    const result = await pool.query(
      `SELECT
       ...
       FROM ai_decisions
       WHERE crm_owner = $1 AND status = 'completed' AND created_at BETWEEN $2 AND $3`,
      [owner_id, start, end]
    );
    return {
