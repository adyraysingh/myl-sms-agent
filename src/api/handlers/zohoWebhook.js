...
async function handleNewLead(webhookData) {
  ...
  // GUARD: Do NOT send any AI outreach to onboarded clients (converted to Account/Contact)
  // is_onboarded is set by onboardingWebhook and dealWebhook when lead becomes a client
  if (lead.is_onboarded === true) {
            logger.info('Lead is already onboarded — skipping AI outreach and follow-ups', {
                        leadId: lead.id,
                        phone: lead.phone,
  ...
  // Also skip if lead_status indicates they are already a client
  const clientStatuses = ['Onboarded', 'Client', 'Active Client', 'Converted'];
        if (lead.lead_status && clientStatuses.includes(lead.lead_status)) {
                  logger.info('Lead status indicates existing client — skipping AI outreach', {
                              leadId: lead.id,
                              leadStatus: lead.lead_status
                  });
                  return;
  ...
  // Trigger AI pipeline: pass normalized id so ZohoIngestor can find/create lead_memory
  ZohoIngestor.ingestLead({ ...webhookData, id: leadData.zohoLeadId }).catch(e => logger.error('ZohoIngestor error', { error: e.message }));
...
function extractLeadData(webhookData) {
        ...
        return {
                  zohoLeadId: data.id || data.lead_id || data.ID || data.Lead_Id,
