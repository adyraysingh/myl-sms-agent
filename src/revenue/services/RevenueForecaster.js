...
class RevenueForecaster {
  ...
  static async runForecast(periodType, periodStart, periodEnd) {
      ...
      const aiResult = {
              ...
              key_drivers: ['Lead count: ' + data.leads.length, 'Hot leads: ' + metrics.breakdown.hot, 'Win rate: ' + Math.round(data.historicalWinRate*100) + '%'],
              risks: ['Pipeline conversion rate'], opportunities: ['Warm lead nurturing'],
              recommended_actions: ['Follow up with hot leads', 'Qualify warm leads'],
              confidence_explanation: 'Based on historical win rate and pipeline composition.'
      };
        const forecast = await ForecastModel.upsert({
                ...
                forecast_narrative: aiResult.forecast_narrative,
                key_drivers: aiResult.key_drivers, risks: aiResult.risks,
                opportunities: aiResult.opportunities, recommended_actions: aiResult.recommended_actions,
        ...
        setImmediate(() => PredictionPublisher.revenue({
                ...
                risks: aiResult.risks, opportunities: aiResult.opportunities,
