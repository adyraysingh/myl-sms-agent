...
return res.json({ success: true, forecast: result.forecast, scenarios, data_summary: result.data_summary, generated_fresh: true });
}
const scenarios = await ForecastModel.getScenariosForForecast(forecast.forecast_id);
res.json({ success: true, forecast, scenarios, generated_fresh: false });
} catch (err) {
...
scenarios = await ForecastModel.listScenarios({ scenario_type, limit: parseInt(req.query.limit || '50'), offset: parseInt(req.query.offset || '0') });
}
res.json({ success: true, scenarios, count: scenarios.length, retrieved_at: new Date().toISOString() });
} catch (err) {
...
try {
const history = await ForecastModel.listHistory({ forecast_type: req.query.forecast_type, limit: parseInt(req.query.limit || '30'), offset: parseInt(req.query.offset || '0') });
const variance = await ForecastModel.getVarianceSummary();
res.json({ success: true, history, variance_summary: variance, count: history.length, retrieved_at: new Date().toISOString() });
} catch (err) {
...
res.json({ success: true, variance_by_type: variance, recent_evaluations: history, retrieved_at: new Date().toISOString() });
} catch (err) {
...
res.json({ success: true, opportunities, count: opportunities.length, retrieved_at: new Date().toISOString() });
} catch (err) {
...
res.json({ success: true, risks, count: risks.length, retrieved_at: new Date().toISOString() });
} catch (err) {
...
const types = forecast_type ? [forecast_type] : ['daily', 'weekly', 'monthly', 'quarterly', 'rolling_30', 'rolling_90'];
res.status(202).json({ success: true, message: 'Forecast recalculation started for: ' + types.join(', '), types, started_at: new Date().toISOString() });
setImmediate(async () => {
for (const t of types) {
try {
...
actual_revenue: parseFloat(actual_revenue || 0),
actual_onboardings: parseInt(actual_onboardings || 0),
revenue_variance: result ? result.revenue_variance : null,
revenue_accuracy: result ? result.revenue_accuracy : null,
evaluated_at: new Date().toISOString(),
notes: notes || null
},
is_correct: result && result.revenue_accuracy !== undefined ? result.revenue_accuracy >= 70 : null,
accuracy_score: result && result.revenue_accuracy !== undefined ? (result.revenue_accuracy / 100) : null,
notes: 'Revenue forecast evaluated: actual $' + (actual_revenue || 0),
source: 'revenue_evaluate_api'
}).catch(e => console.error('[OutcomeLinker] revenue_received failed:', e.message)));
res.json({ success: true, evaluation: result, evaluated_at: new Date().toISOString() });
} catch (err) {
...
success: true,
dashboard: {
revenue_forecast: monthly ? { expected_revenue: monthly.expected_revenue, expected_onboardings: monthly.expected_onboardings, confidence: monthly.confidence, period_start: monthly.period_start, period_end: monthly.period_end } : null,
weekly_forecast: weekly ? { expected_revenue: weekly.expected_revenue, expected_onboardings: weekly.expected_onboardings, confidence: weekly.confidence } : null,
quarterly_forecast: quarterly ? { expected_revenue: quarterly.expected_revenue, expected_onboardings: quarterly.expected_onboardings, confidence: quarterly.confidence } : null,
