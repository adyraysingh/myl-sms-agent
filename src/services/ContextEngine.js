 ...
 *
 * A lead that has just been created contains only a name, email, and phone.
 * That is not enough evidence for the AI to produce a meaningful qualification
 * score or a business decision. Sending it anyway wastes tokens and produces
 * hallucinated "confident" recommendations based on zero real interaction.
 *
 * ContextEngine solves this by:
 *  1. Querying every signal table for the lead.
 *  2. Scoring how much evidence is available (0-100).
 *  3. Identifying exactly which signals are missing.
 *  4. Returning { ready: true/false, evidenceScore, missingSignals, context }
 *     so callers can decide whether to proceed or return "Insufficient Information".
 *
...
class ContextEngine {
  ...
  static async _gatherSignals(leadId, zohoLeadId) {
    const queries = await Promise.allSettled([
      ...
      // 2: retell calls
      pool.query(
        'SELECT * FROM retell_calls WHERE lead_id = $1 ORDER BY created_at DESC LIMIT 10',
        [leadId]
      ),
      // 3: email events
      pool.query(
        'SELECT * FROM email_events WHERE lead_id = $1 ORDER BY created_at DESC LIMIT 10',
        [leadId]
      ),
      // 4: crm tasks
      pool.query(
        'SELECT * FROM crm_tasks WHERE lead_id = $1 ORDER BY created_at DESC LIMIT 10',
        [leadId]
      ),
      // 5: crm notes
      pool.query(
        'SELECT * FROM crm_notes WHERE lead_id = $1 ORDER BY created_at DESC LIMIT 10',
        [leadId]
      ),
      // 6: lead events timeline
      pool.query(
        'SELECT * FROM lead_events WHERE lead_id = $1 ORDER BY created_at DESC LIMIT 20',
        [leadId]
      ),
      // 7: existing qualification
      pool.query(
 ...
 * Callers catch this and return "Insufficient Information" responses
 * instead of running the AI engine.
 */
