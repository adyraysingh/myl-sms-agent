...
const FOLLOW_UP_MESSAGES = [
  function(name) { return "Hey " + name + "! Just checking in - have you had a chance to think about launching your clothing brand? Happy to answer any questions. - Maya from MakeYourLabel"; },
  function(name) { return "Hi " + name + ", Maya here from MakeYourLabel. I'd love to help you get your brand off the ground. What questions do you have?"; },
  function(name) { return name + ", still thinking about launching? We help founders launch without manufacturing headaches. Worth exploring? https://start.makeyourlabel.com"; },
  function(name) { return "Last check-in from me, " + name + ". When you're ready to start your clothing brand, I'm here. https://start.makeyourlabel.com"; }
];

async function scheduleFollowUps(leadId, conversationId, type) {
  ...
  try {
    ...
    for (var i = 0; i < FOLLOW_UP_SCHEDULE.length; i++) {
      var schedule = FOLLOW_UP_SCHEDULE[i];
      var scheduledAt = new Date(Date.now() + schedule.delayMinutes * 60 * 1000);
      await query(
    ...
    logger.info('Follow-ups scheduled', { leadId: leadId, count: FOLLOW_UP_SCHEDULE.length });
  } catch (error) {
...
async function cancelFollowUps(leadId) {
  try {
    ...
    logger.info('Follow-ups cancelled', { leadId: leadId });
  } catch (error) {
...
async function processFollowUps() {
  try {
    ...
    for (var i = 0; i < result.rows.length; i++) {
      var followUp = result.rows[i];
      try {
        var name = followUp.first_name || 'there';
        var msgIdx = Math.min(followUp.sequence_number - 1, FOLLOW_UP_MESSAGES.length - 1);
        var message = FOLLOW_UP_MESSAGES[msgIdx](name);
...
function startFollowUpScheduler() {
  cron.schedule('* * * * *', async function() { await processFollowUps(); });
  logger.info('Follow-up scheduler started');
}

module.exports = { scheduleFollowUps: scheduleFollowUps, cancelFollowUps: cancelFollowUps, startFollowUpScheduler: startFollowUpScheduler };
